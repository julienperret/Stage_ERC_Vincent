...
setwd("/home/chap/encadrement/repoJulien/erc/traitements_Stats/Fitting_distrib_surfaces_etages/")

df <-  read_csv("surface_sol.csv")

#les codes iris sont des facteurs
df$CODE_IRIS <- factor(df$CODE_IRIS)
...
psurf <-  ggplot(df, aes(SURFACE_SOL))+
...
psurf2 <-  ggplot(df, aes(SURFACE_SOL))+
...
dfSurfByIRIS <- df %>% group_by(CODE_IRIS) %>% summarize(Surf_Tot=sum(SURFACE_SOL)) %>% arrange(desc(Surf_Tot))

# réordonne les niveaux de facteurs   
dfSurfByIRIS$CODE_IRIS <- factor(dfSurfByIRIS$CODE_IRIS, levels = dfSurfByIRIS$CODE_IRIS[order(-dfSurfByIRIS$Surf_Tot)])



#allure de la distribution globale 

p<- ggplot(dfSurfByIRIS, aes( CODE_IRIS,Surf_Tot  ))+
...
ppp<- ggplot(head(dfSurfByIRIS,30), aes( CODE_IRIS,Surf_Tot))+
  geom_bar(fill="darkolivegreen2", colour="darkgrey", stat = "identity")+
...
pp<- ggplot(tail(dfSurfByIRIS,30), aes( CODE_IRIS,Surf_Tot))+
  geom_bar(fill="darkolivegreen2", colour="darkgrey", stat = "identity")+
...
IRISmaousse <-  dfSurfByIRIS$CODE_IRIS[1]
dbig <-  df %>% filter(CODE_IRIS== IRISmaousse)

pmaousse <-  ggplot(dbig, aes(SURFACE_SOL))+
  geom_histogram( fill="darkolivegreen2", colour="darkgrey", binwidth = 1)+
  labs(x="Surface ", y="effectif")+
  scale_x_continuous(breaks = seq(0,max(dbig$SURFACE_SOL), by = 1000))+
...
dfSurfDistinct <- df %>% group_by(CODE_IRIS) %>% summarize(Surf_Tot= sum(SURFACE_SOL),distcount=n_distinct(SURFACE_SOL)) %>% arrange(desc(distcount))

# réordonne les niveaux de facteurs   par nombre de hauteurs distinctes
dfSurfDistinct$CODE_IRIS <- factor(dfSurfDistinct$CODE_IRIS, levels = dfSurfDistinct$CODE_IRIS[order(-dfSurfDistinct$distcount)])


IRISvarious <- dfSurfDistinct$CODE_IRIS[1]

dvarious <- df %>% filter(CODE_IRIS==IRISvarious) 

pvarious <-  ggplot(dvarious, aes(SURFACE_SOL))+
  geom_histogram( fill="darkolivegreen2", colour="darkgrey", binwidth = 1)+
  labs(x="Surfaces", y="effectif")+
  scale_x_continuous(breaks = seq(0,max(dvarious$SURFACE_SOL), by = 100))+
...
dSurfNormExist  <- df  %>% group_by(CODE_IRIS) %>%  count(round(SURFACE_SOL))  %>% mutate(SurfNorm = n / sum(n) )
names(dSurfNormExist) <-  c("CODE_IRIS", "SURFACE", "effectif", "Surface_normalisee_a_lIRIS" )

# ecriture du fichier de poids
write_csv(dSurfNormExist,path = "poids_Surfaces_Observees_Normalises_parIRIS.csv")
...
dd <- df %>% filter(CODE_IRIS==IRISvarious) 
...
for (candid in candidats) {
  ...
  for (met in methods) {
   # cat("methode : " , met, "\n")
    tryCatch({
      fit <-  fitdist(dd$SURFACE_SOL, distr = candid, method=met, discrete = F)
      gof <-  gofstat(fit)
...
cptr <- 0
for (c in unique(df$CODE_IRIS)) {

  distribObservee <-  df %>% filter(CODE_IRIS == c)
  ...
  fittedDistros$CODE_IRIS <- c
  
  cptr  <-  cptr + 1
  cat(cptr, "/", length(unique(df$CODE_IRIS)),"IRIS:", c ,"\n")
...
getwd()
write.csv(distribsResults, "poidsSurface_fittes_by_IRIS_SUMTO1.csv")
